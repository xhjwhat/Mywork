...
public class XListView extends ListView implements OnScrollListener {
	...
	private void initWithContext(Context context) {
		...
		mFooterView.hide();
//Modify by Davee on 2014-11-19 start
		// init header height
...
//					}
//				});
		mHeaderViewHeight = mHeaderViewContent.getLayoutParams().height;
//Modify by Davee on 2014-11-19 end
	}
	...
	public void stopLoadMore() {
		if (mPullLoading == true) {
			mPullLoading = false;
			if (mAutoLoadMore) mFooterView.hide();
			mFooterView.setState(XListViewFooter.STATE_NORMAL);
		}
	}
	...
	public void startLoadMore() {
		if (!mPullLoading) {
			mPullLoading = true;
			if (mAutoLoadMore) mFooterView.show();
...
//Add by Davee on 2014-11-19 start
	private void refresh(boolean flag) {
		if (mEnablePullRefresh && !mPullRefreshing) {
			mPullRefreshing = true;
			mHeaderView.setState(XListViewHeader.STATE_REFRESHING);
			if (mListViewListener != null && flag) {
				mListViewListener.onRefresh();
			}
			updateHeaderHeight(mHeaderViewHeight + 1);
...
//Add by Davee on 2014-11-19 end
