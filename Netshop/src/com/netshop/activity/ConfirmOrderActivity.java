...
public class ConfirmOrderActivity extends Activity implements OnClickListener{
	...
	public void onCreate(Bundle bundle){
		...
		selectedList = (ArrayList<Product>) getIntent().getSerializableExtra("product");
			...
			defaultRequest.request(new HttpCallBack() {
				public void success(String json) {
					...
					if(entity!=null){
						...
						addrText.setText(entity.getAddress());
						numText.setText(entity.getPhone());
					}else{
						Toast.makeText(ConfirmOrderActivity.this, "没有默认地址，先选一个地址吧", Toast.LENGTH_SHORT).show();
					}
				}
		...
		case R.id.confirm_btn:
			...
			for(int i=0;i<selectedList.size();i++){
				if(i== selectedList.size()-1){
					buffer.append(selectedList.get(i).getPid()+","+selectedList.get(i).getNum());
				}else{
					buffer.append(selectedList.get(i).getPid()+","+selectedList.get(i).getNum()+";");
				}
			}
			...
			request.request(new HttpCallBack() {
				public void success(String json) {
					BaseEntity entity = new Gson().fromJson(json, BaseEntity.class);
				...
				public void fail(String failReason) {
					Toast.makeText(ConfirmOrderActivity.this, failReason, Toast.LENGTH_SHORT).show();
				}
			});
		}
