...
public class ShopCommentActivity extends Activity {
	...
	public void onCreate(Bundle bundle){
		...
		if(intent!=null){
			shopid=intent.getStringExtra("key");
		}
		...
		HttpRequest request = new HttpRequest("4", "0004");
		request.setPc(shopid);
		request.setPg("1");
		request.setPs("20");
...
//				json = xmls.read(result).toString().replace("@", "");
				...
				if(temp == null){
					Toast.makeText(ShopCommentActivity.this, "没有评论数据", Toast.LENGTH_SHORT).show();
					return;
					//finish();
				}
				...
				adapter = new CommentAdapter(ShopCommentActivity.this, commentList);
				list.setAdapter(adapter);
			}
			
			@Override
			public void fail(String failReason) {
				Toast.makeText(ShopCommentActivity.this, failReason, Toast.LENGTH_SHORT).show();
				finish();
			}
		});
	}
